...
public class UseFeignApi {
    ...
    /**
     * description:使用feign 的原生api调用远程服务
     *      Feign 默认配置初始化，设置自定义配置，生成代理对象
     * @param
     * @return {@link JwtToken}
     * @author: feather
     * @since: 2024-03-06 19:59
     **/
    public JwtToken thinkingInFeign(UserNameAndPassword userNameAndPassword){
        ...
        //如果服务id不存在，直接抛异常
        if (null==serviceId){
            throw  new BusinessException("can not get serviceId");
        }
        ...
        if (CollectionUtils.isEmpty(targetInstances)){
            throw  new BusinessException("can not get target instance from serviceId:[{}]",serviceId);
        }
        //随机选择一个服务实例
